...
export interface INotebookOriginalModelReferenceFactory {
	...
	getOrCreate(fileEntry: any, viewType: string): Promise<IReference<NotebookTextModel>>;
}
...
export class OriginalNotebookModelReferenceCollection extends ReferenceCollection<Promise<NotebookTextModel>> {
	...
	protected override async createReferencedObject(key: string, fileEntry: any, viewType: string): Promise<NotebookTextModel> {
